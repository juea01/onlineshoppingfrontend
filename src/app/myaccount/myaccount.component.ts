...
export class MyaccountComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (window.innerWidth < 992) {
      let menuPannel = document.getElementById('myaccount-menu');
      let careerbutton = document.getElementById('menu-button');
      menuPannel.style.display = 'none';
      careerbutton.innerHTML = 'Open MyAccount Menu...>';
    } else {
      let menuPannel = document.getElementById('myaccount-menu');
      let careerbutton = document.getElementById('menu-button');
      menuPannel.style.display = 'block';
      careerbutton.innerHTML = 'Close MyAccount Menu...X';
    }
  }
  ...
  toogleCareerPanel() {
    ...
    if (
      menuPannel.style.display === 'none' ||
      menuPannel.style.display === ''
    ) {
      menuPannel.style.display = 'block';
      careerbutton.innerHTML = 'Close MyAccount Menu...X';
    } else {
      menuPannel.style.display = 'none';
      careerbutton.innerHTML = 'Show MyAccount Menu...>';
    }
  }
  ...
  logout() {
    ...
    this.keycloak
      .logout(`${docker_env_config.keycloakRedirectUrl}`)
      .then(() => {
    ...
    this.subscriptionRepo
      ...
      .subscribe((result) => {
        if (result.redirectView) {
          ...
          this.errorMessage =
            "You can't manage billing as you don't have active payment subscription yet.";
        }
      });
  }
  ...
  navigateToUserDetail() {
    ...
    this.router.navigate(['/myaccount/main/userdetails']);
  }
}
